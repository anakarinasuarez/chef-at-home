...
export const useRecipesGenerationZustand = (): UseRecipesGenerationReturn => {
  const recipes = useRecipes();
  const hasLoadedRecipes = useHasLoadedRecipes();
  const loading = useIsLoading();
  const error = useError();
  const { setRecipes, setHasLoadedRecipes, setLoading, setError } =
    useAppActions();
  ...
  }, [hasLoadedRecipes, setRecipes, setHasLoadedRecipes, setLoading, setError]);
  ...
  }, [setRecipes, setHasLoadedRecipes, setLoading]);
