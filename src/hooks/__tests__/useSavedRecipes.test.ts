...
describe("useSavedRecipes", () => {
  ...
  describe("Save Recipe", () => {
    ...
    it("handles localStorage errors gracefully", () => {
      ...
      // The recipe is still added to state even if localStorage fails
  ...
  describe("Update Recipe", () => {
    it("updates an existing recipe successfully", () => {
      ...
      act(() => {
        const success = result.current.updateRecipe(
          mockRecipe.id!,
          updatedData
        );
        expect(success).toBe(true);
    ...
    it("returns false when recipe not found for update", () => {
      ...
      act(() => {
        const success = result.current.updateRecipe("non-existent-id", {
          title: "New Title",
        });
        expect(success).toBe(false);
      });
    ...
    it("returns false when no user is logged in", () => {
      ...
      act(() => {
        const success = result.current.updateRecipe(mockRecipe.id!, {
          title: "New Title",
        });
        expect(success).toBe(false);
      });
  ...
  describe("User Changes", () => {
    it("reloads recipes when user changes", () => {
      ...
      // Force reload by calling loadSavedRecipes
      act(() => {
        result.current.loadSavedRecipes();
      });
  ...
  describe("Performance", () => {
    ...
    it("handles large number of recipes efficiently", () => {
      ...
      vi.mocked(localStorage.getItem).mockReturnValue(
        JSON.stringify(largeRecipeList)
      );
