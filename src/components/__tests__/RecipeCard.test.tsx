...
describe('RecipeCard', () => {
  ...
  describe('User Interactions', () => {
    it('navigates to recipe detail on card click', () => {
      ...
      const card = screen.getAllByText('Test Recipe')[0].closest('div')?.closest('div');
    ...
    it('navigates to recipe detail with from parameter for my-recipes variant', () => {
      ...
      expect(mockPush).toHaveBeenCalledWith('/recipes/test-recipe-id?from=my-recipes');
    });
    ...
    it('generates unique ID when recipe has no ID', () => {
      ...
      expect(mockPush).toHaveBeenCalledWith(expect.stringMatching(/^\/recipes\/\d+$/));
    });
  });
