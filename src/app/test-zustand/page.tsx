...
export default function TestZustandPage() {
  ...
  const runTests = useCallback(async () => {
    ...
    try {
      ...
      addTestResult(`✅ Test 2: Estado inicial - Recetas: ${currentState.recipes.recipes.length}`);
      addTestResult(
      ...
      if (stateAfterRecipe.recipes.recipes.some(r => r.id === 'test-recipe-1')) {
      ...
      if (stateAfterSaved.savedRecipes.savedRecipes.some(r => r.id === 'test-saved-recipe-1')) {
    ...
    <MainLayout currentPage='create'>
      ...
      >
        <div className='max-w-4xl mx-auto'>
          <h1 className='text-3xl font-bold mb-8' style={{ color: colors.interface.text.primary }}>
          ...
          <div className='grid grid-cols-1 md:grid-cols-3 gap-6 mb-8'>
            ...
            >
              ...
              <div className='space-y-2 text-sm'>
                <div>
                  Usuario:{' '}
                  {getCurrentState().auth.user ? getCurrentState().auth.user?.name : 'No logueado'}
                </div>
                <div>Loading: {getCurrentState().auth.isLoading ? 'Sí' : 'No'}</div>
            ...
            >
              ...
              <div className='space-y-2 text-sm'>
                ...
                <div>Loading: {getCurrentState().recipes.isLoading ? 'Sí' : 'No'}</div>
                <div>Cargadas: {getCurrentState().recipes.hasLoadedRecipes ? 'Sí' : 'No'}</div>
                <div>Índice activo: {getCurrentState().recipes.activeIndex}</div>
              </div>
            </div>
            ...
            >
              ...
              <div className='space-y-2 text-sm'>
                ...
                <div>Loading: {getCurrentState().recipesGeneration.isLoading ? 'Sí' : 'No'}</div>
                <div>
                  Cargadas: {getCurrentState().recipesGeneration.hasLoadedRecipes ? 'Sí' : 'No'}
                </div>
            ...
            >
              ...
              <div className='space-y-2 text-sm'>
                <div>Cantidad: {getCurrentState().savedRecipes.savedRecipes.length}</div>
                <div>Loading: {getCurrentState().savedRecipes.isLoading ? 'Sí' : 'No'}</div>
                <div>
                  Eliminando: {getCurrentState().savedRecipes.removingRecipeId || 'Ninguna'}
                </div>
              </div>
            </div>
          </div>
            ...
            <div className='space-y-1 text-sm font-mono max-h-96 overflow-y-auto'>
              {testResults.length === 0 ? (
                ...
                testResults.map((result, index) => (
                  <div key={index} style={{ color: colors.interface.text.primary }}>
                    {result}
                  </div>
                ))
              )}
